...
-- 4. Add interviewing date column
ALTER TABLE public.applications ADD COLUMN IF NOT EXISTS interview_date DATE;

-- 5. Set up Row Level Security (RLS) if not already set
ALTER TABLE public.applications ENABLE ROW LEVEL SECURITY;

-- 6. Create policies (first drop existing to avoid conflicts)
